  ...
  try {
    ...
    await doc.save(function(err, result) {
      if (err) console.log(err);
        ...
        res.status(201).send({_id:id,summed: _(result._doc.consumazioni)
            .groupBy('nameArticle')
            .map((objs, key) => {
              return {
                'store': key,
            ...
            .value(), total: _.sumBy(result._doc.consumazioni, 'price' )});
      }
    });
  } catch (error) {
...
router.put("/api/customer", auth, async (req, res) => {
  try {
    ...
    let doc = await customerModel.findOneAndUpdate(filter,
      {$push: {consumazioni: consumazioni}}, {new: true},
      );
    res.status(201).send({ id:doc._doc._id,summed: _(doc._doc.consumazioni)
        .groupBy('nameArticle')
        .map((objs, key) => {
          return {
            'store': key,
        ...
        .value(),total: _.sumBy(doc._doc.consumazioni, 'price' )
      });
...
router.put("/api/customer/close", auth, async (req, res) => {
  try {
    const filter = { _id: req.body._id };
    const doc = await customerModel.findOneAndUpdate(filter, {$set: {statusTable: 0}});
...
router.get("/api/customer", auth, async (req, res) => {
  try {
    ...
    if(items && items._doc.consumazioni && items._doc.consumazioni.length > 0){
      res.status(200).send({ summed :_(items._doc.consumazioni)
          .groupBy('nameArticle')
          .map((objs, key) => {
            return {
              'store': key,
          ...
          .value(), _id:items._doc._id, total: _.sumBy(items._doc.consumazioni, 'price' ) })
    }else{
      res.status(200).send({ summed :[], _id: null, total: 0.00})
    }
