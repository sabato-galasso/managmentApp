...
export class MenuCategoryComponent implements OnInit {
  ...
  loadData() {
    ...
    this.subscriptionGetItems = this.menuService.getCategories().subscribe(
      (items) => {
        this.showSpinner = true
  ...
  updateDataTableValue(data) {
    ...
    this.subscriptionFilterUpdateItems = this.menuService
      .updateCategory(data)
      .subscribe(
        (items) => {
          this.showSpinner = true
  ...
  addRowData(rowObj: MenuCategoryResponse) {
    ...
    if (rowObj.nameFirstLevel) {
      rowObj.slugCategoryFirst =
        this.slugifyPipe.transform(rowObj.nameFirstLevel) || ''
    }
    this.menuService.addCategory(rowObj).subscribe(
      (items) => {
        this.showSpinner = true
      },
      (errMess) => {
  ...
  deleteRowData(rowObj: MenuCategoryResponse) {
    this.subscriptionDeleteItems = this.menuService
