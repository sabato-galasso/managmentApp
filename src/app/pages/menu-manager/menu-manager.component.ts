...
export class MenuManagerComponent implements OnInit {
  ...
  displayedColumns: string[] = ['_id', 'name', 'category', 'categoryFirstLevel', 'price', 'updated_at', 'actions'];
  errMessFeed: string;
  showSpinner = false;
  ...
  updateDataTableValue(data: MenuResponse) {
    ...
    this.menuService.updateMenu(data).subscribe(items => {
        this.showSpinner = true;
  ...
  addRowData(rowObj: MenuResponse) {
    ...
    if(rowObj.nameFirstLevel)
      rowObj.slugCategoryFirstLevel = this.slugifyPipe.transform(rowObj.nameFirstLevel)

    this.menuService.addMenu(rowObj).subscribe(items => {
        this.showSpinner = true;
      },
      errMess => {
        this.errMessFeed = errMess as any;
  ...
  deleteRowData(rowObj: MenuResponse) {
    ...
    this.menuService.deleteMenuRow(rowObj).subscribe(items => {
        this.showSpinner = true;
      },
      errMess => { this.errMessFeed = errMess as any;
        this.openSnackBar(this.errMessFeed, 'Undo');
